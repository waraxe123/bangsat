...
from SiestaRobot.function.pluginhelpers import get_text
...
@pbot.on_message(filters.command("nulis1") & ~filters.edited & ~filters.bot)
async def nulis1(client, message):
...
@pbot.on_message(filters.command("nulis2") & ~filters.edited & ~filters.bot)
async def nulis2(client, message):
    if message.reply_to_message:
        try:
            msg = await client.send_message(message.chat.id, "**Writing the text....**")
        except:
